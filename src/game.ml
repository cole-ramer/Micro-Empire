  ...
  let nearest_nutrient nutrients (avg_colony_position : Position.t) =
    Set.fold (Position.Set.union_list nutrients) ~init:(Int.max_value, None)
      ~f:(fun (best_distance, best_position_option) nutrient ->
        ...
        match distance < best_distance with
        | true -> (distance, Some nutrient)
        | false -> (best_distance, best_position_option))

  let nearest_smaller_colony enemy_map average_position_map ~enemy_id =
    ...
    let distance, nearest_smaller_enemy_id =
      Map.fold enemy_map ~init:(Int.max_value, None)
        ~f:(fun ~key ~data (best_distance, best_enemy_option) ->
          if key = enemy_id then (best_distance, best_enemy_option)
          else
            ...
            with
            | true, true -> (current_distance, Some key)
            | _, _ -> (best_distance, best_enemy_option))
    in
    (distance, nearest_smaller_enemy_id)
          ...
          raise_s
            [%message
              "There must be at least one nutrient the colony could move \
               towards"]
    in
    let closest_enemy_distance, closest_enemy_id =
      nearest_smaller_colony game.enemies avg_position_map ~enemy_id
    in
    match
      (closest_nutrient_distance > closest_enemy_distance, closest_enemy_id)
    with
    | true, Some id ->
        move_and_upgrade_colony ~game ~enemy_id ~colony:moving_colony
          ~source:enemy_avg_position
          ~target:(Map.find_exn avg_position_map id)
